...
// GETS A SINGLE DRIVER FROM THE DATABASE USING LAT
router.get('/:latitude', function (req, res) {
	Driver.findByLatitude(req.params.latitude, function (err, user) {
		if (err) return res.status(500).send("There was a problem finding the user.");
		if (!user) return res.status(404).send("No driver found.");
		res.status(200).send(user);
	});
...
// GETS A SINGLE DRIVER FROM THE DATABASE USING LONG
router.get('/:longitude', function (req, res) {
	Driver.findByLongitude(req.params.longitude, function (err, user) {
		if (err) return res.status(500).send("There was a problem finding the user.");
		if (!user) return res.status(404).send("No driver found.");
		res.status(200).send(user);
	});
